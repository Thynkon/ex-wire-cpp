...
export module PcapWrapper;

using namespace std;

export class PcapWrapper {
private:
...
public:
  ...
  PcapWrapper(string_view device, string_view filter) {
    this->device = device;
    this->filter = filter;
    this->packet_count = 0;
  }
  ...
  bool is_closed() const { return this->handle == nullptr; }

  ::pcap_stat get_stats() const {
